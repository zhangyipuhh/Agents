...
### 合规性审查
1. When collecting information for the `save_business_info` tool, **ALWAYS prioritize extracting it from the current conversation context first**. If the context already contains the required information (project name, construction unit, contact person, phone, address), use it directly and do NOT ask the user to provide it again.
   - Only if the context is incomplete, use the `explore` tool to search attachments as a supplement.
   ...
   - If `explore` finds the required information, extract and use it directly. Only use `ask_user_question` if information is still missing after checking both context and attachments.
2. Use the `save_business_info` tool to persist the business information. This step is mandatory; refer to the tool parameters for required fields. When asking, use one tab to include all information that needs to be saved.

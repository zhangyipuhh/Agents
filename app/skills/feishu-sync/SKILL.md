...
- 接收消息类型路由：
  ...
  - `msg_type=file`，后缀不在白名单内 → 自动回发提示文本「暂不支持的文件类型」
  - 其他类型（image / post / audio / sticker 等）→ 仅记录日志，跳过处理
- 文件消息处理流程（后缀在白名单时）：
  1. 通过 `client.im.v1.message_resource.get(message_id, file_key, type="file")` 下载原始字节
  2. 落到 `data/upload/yyyy/mm/dd/{session_marker}/`（由 `session_path_manager` 提供，session_marker = session_id 中 `:` 替换为 `_`，Windows 文件系统兼容）
  3. 按 `settings.file_parser.file_parser_enabled` 选择 `FileParserClient.parse()` 或 `DocumentLoader.load()` 解析为 `.md`，并落 `data/tmp/upload/yyyy/mm/dd/{session_marker}/{stem}.md` 镜像
  4. `.md / .txt` 直接读本地内容并复制为 `.md` 镜像；`docx / pdf / xlsx` 走文件解析器
  5. 大小限制取 `settings.file_parser.file_parser_max_file_size` 与飞书官方 100MB 隐式限制的较小值
  6. user text **仅含文件名列表**——写入形如 `用户上传了以下文件：\n- <file_name>\n- ...`，**不**暴露路径、解析镜像、preview、文件正文。文件实际仍按保存在 `data/upload/<safe_marker>/` 与 `data/tmp/upload/<safe_marker>/<stem>.md`；agent 通过 `explore / query_knowledge / file_read` 等工具按文件名按需读取
  7. 失败回执（白名单外 / 超大 / 下载）→ 走现有 `_send_text_reply` 路径，告诉用户原因，**不**触发 agent；解析失败但文件已落盘 → 把文件名纳入上方列表后让 agent 继续按需读取
- 转交智能体后回复渲染路由：

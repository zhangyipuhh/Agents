...
- 文件消息处理流程（后缀在白名单时）：
  ...
  6. user text **仅含元数据**——把 "原文路径 / 解析镜像路径 / 最多 200 字 preview" 写成 YAML 风格块发给 agent。**不**把整篇文件 inline 进 user text，agent 通过 `explore / query_knowledge / file_read` 等工具按需读取路径
  7. 失败回执（白名单外 / 超大 / 下载）→ 走现有 `_send_text_reply` 路径，告诉用户原因，**不**触发 agent；解析失败但文件已落盘 → 把原文路径交给 agent，让其按需读取
- 转交智能体后回复渲染路由：

---
name: feishu-sync
description: "封装飞书 Open API 主动推送消息、文档或卡片；并通过 WebSocket 长连接被动接收私聊与群聊 @机器人 消息，转交 project 智能体处理后回复。"
---

## Keywords (关键词)

- 飞书 (feishu)
- 飞书通知 (feishu-notify)
- 飞书同步 (feishu-sync)
- 飞书消息 (feishu-message)
- 飞书文档 (feishu-doc)
- 飞书卡片 (feishu-card)
- 飞书机器人 (feishu-bot)
- 飞书 WebSocket (feishu-ws)
- 飞书文件附件 (feishu-file-attachment)
- 飞书 docx/pdf/xlsx 上传 (feishu-document-upload)
- markdown 渲染 (markdown-render)
- 卡片渲染 (card-render)
- 人工回路 (hitl)
- HITL 按钮 (hitl-button)
- 中断按钮卡片 (interrupt-card)

## 能力清单

### 主动推送（agent 调用）
- `send_feishu_message` 工具：向飞书群/用户发送文本消息

### 被动接收（系统级服务，非工具）
- `FeishuWebSocketService`：随 FastAPI lifespan 启停
- 订阅 `im.message.receive_v1` 事件（私聊全部回复，群聊仅响应 @机器人）
- 订阅 `card.action.trigger` 事件（HITL 按钮点击回调）
- 接收消息类型路由：
  - 文本消息 → 直接转交 `settings.feishu.feishu_ws_agent_name` 指定的智能体处理
  - `msg_type=file`，后缀在白名单 `docx / pdf / xlsx / md / txt` 内 → 走下载→解析→注入 user text 路径（见下），再转交 agent
  - `msg_type=file`，后缀不在白名单内 → 自动回发提示文本「暂不支持的文件类型」
  - 其他类型（image / post / audio / sticker 等）→ 仅记录日志，跳过处理
- 文件消息处理流程（后缀在白名单时）：
  1. 通过 `client.im.v1.message.get_message_resource(message_id, file_key, type="file")` 下载原始字节
  2. 落到 `data/upload/yyyy/mm/dd/{session_id}/`（由 `session_path_manager` 提供）
  3. 按 `settings.file_parser.file_parser_enabled` 选择 `FileParserClient.parse()` 或 `DocumentLoader.load()` 解析为 `.md`
  4. `.md / .txt` 直接读本地内容；`docx / pdf / xlsx` 走文件解析器
  5. 大小限制取 `settings.file_parser.file_parser_max_file_size` 与飞书官方 100MB 隐式限制的较小值
  6. 解析结果与用户伴随文本一起作为 `<attached file: name>\n<md_content>\n\n[用户文本] ...` 注入 user message
  7. 失败回执（白名单外 / 超大 / 下载 / 解析）→ 走现有 `_send_text_reply` 路径，告诉用户原因，**不**触发 agent
- 转交智能体后回复渲染路由：
  - Markdown 自动检测 → 转飞书交互式卡片（`MarkdownToCardConverter`）
  - 纯文本 → 直接发送
- HITL 中断 → 发带选项按钮的交互式卡片（`InterruptToCardConverter`）
- 卡片 API 失败 → 自动降级纯文本发送
- HITL 按钮回调 → 解析 `qid` / `oid` → `Command(resume={"answers": [...]})` 续跑 agent

## 依赖关系

- 被依赖方：`requirement-ticket` / `change-ticket` / `ops-inspection` 调用 `send_feishu_message`
- 配置依赖：`feishu_app_id` / `feishu_app_secret` / `feishu_ws_enabled` / `feishu_ws_agent_name`

## 触发关键词

- 通常不直接由用户触发；被动接收模式下由飞书消息事件自动驱动
- 主动推送由 agent 通过 `send_feishu_message` 工具触发

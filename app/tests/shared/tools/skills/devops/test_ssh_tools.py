...
# =============================================================================
# 2026-07-29 统一审计日志契约：所有终态经 LogService.emit(LogEvent) 落地
# =============================================================================
...
def test_execute_command_success_emits_ssh_log_event(monkeypatch):
    ...
    # command_redacted 必须经过 redact_command 处理(原命令无敏感字段时与原文相同,
    # 但若含 --password=xxx / -pxxx 等会被替换)。这里仅校验字段为非空字符串。
...
def test_execute_command_blocked_emits_ssh_log_event(monkeypatch):
    ...
    execute_command(command="rm -rf /tmp/x", business_name="gamma", runtime=runtime)
    ...
    assert isinstance(md["command_redacted"], str)
    assert md["command_redacted"]
    assert md["command_hash"]  # 非空 hash
    # hash 必须对原始命令(redact 前),不能用 redacted 文本
...
def test_execute_command_fail_soft_when_log_service_missing(monkeypatch):
    ...
    out = execute_command(command="echo hi", business_name="delta", runtime=runtime)
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is True
...
def test_execute_command_emits_even_when_emit_raises(monkeypatch):
    ...
    out = execute_command(command="echo hi", business_name="epsilon", runtime=runtime)
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is True
...
def test_execute_command_metadata_never_leaks_original_command(monkeypatch):
    ...
    execute_command(
        command="mysql --password=hunter2xyz -e 'select 1'",
        business_name="zeta2",
        runtime=runtime,
    )
...
def test_execute_command_does_not_emit_stdout_stderr_in_metadata(monkeypatch):
    ...
    _patch_paramiko(
        monkeypatch,
        stdout_text="secret-output-line\n",
    ...
    execute_command(command="echo hi", business_name="eta", runtime=runtime)
    ...
    # output 在 read 后做过 strip() 去除首尾空白,所以 stdout_size 是 stripped 后的字节数。
...
def test_execute_batch_commands_emits_one_summary_and_one_per_command(monkeypatch):
    ...
    # 一次 batch 调用产生 1 条汇总 + 2 条子命令 = 3 条 emit
...
def test_execute_batch_commands_blocked_emits_blocked_and_skipped(monkeypatch):
    ...
    cfg = {
        ...
        "server_type": "linux",
        "blacklist": [r"^shutdown"],
        "whitelist": ["ls", "shutdown -h now", "whoami"],
    }
    _patch_service(monkeypatch, cfg)
    ...
    # 期望 4 条: 1 汇总 + 1 blocked(shutdown) + 2 skipped(ls / whoami)
    ...
    # 拦截项 error_code = blocked, skipped 项 error_code = batch_rejected
...
def test_execute_batch_commands_invalid_list_emits_one_failure(monkeypatch):
    ...
    out = execute_batch_commands(commands=[], business_name="kappa", runtime=runtime)
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is False
...
def test_execute_command_correlation_id_is_none_per_call(monkeypatch):
    ...
    execute_command(command="echo a", business_name="nu", runtime=runtime)
    execute_command(command="echo b", business_name="nu", runtime=runtime)
...
def test_execute_command_blocked_path_emits_no_paramiko_call(monkeypatch):
    ...
    execute_command(command="rm -rf /tmp/x", business_name="xi", runtime=runtime)
...
def test_execute_command_uses_runtime_tool_call_id_when_present(monkeypatch):
    ...
    execute_command(command="echo hi", business_name="omikron", runtime=runtime)
    assert captured[-1].tool_call_id == "call-abc"

    # runtime=None 时退到 unknown
    captured.clear()
    execute_command(command="echo hi", business_name="omikron", runtime=None)
    assert captured[-1].tool_call_id == "unknown"


# =============================================================================
# 2026-07-29 修订：success 语义统一 + intercept_reason 脱敏 + batch 共享 UUID
# =============================================================================
...
def test_execute_command_success_even_when_stderr_noisy(monkeypatch):
    ...
    out = execute_command(
        command="echo hi", business_name="zeta-noise", runtime=runtime
    )
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is True
    # 退出码为 0 → error_code 必须为 None
    evt = captured[-1]
...
def test_execute_command_failure_only_when_exit_nonzero(monkeypatch):
    ...
    out = execute_command(
        command="false", business_name="zeta-fail", runtime=runtime
    )
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is False
    evt = captured[-1]
...
def test_execute_command_blocked_intercept_reason_is_fixed_category(monkeypatch):
    ...
    execute_command(
        command="mysql --password=blocked-secret",
        business_name="redact-test",
        runtime=runtime,
    )
...
def test_execute_batch_commands_auth_failure_emits_summary_and_members(monkeypatch):
    ...
    # 让 _open_client 触发 AuthenticationException
    ...
    out = execute_batch_commands(
        commands=["whoami", "date", "ls"],
        business_name="auth-fail",
        runtime=runtime,
    )
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is False
...
def test_execute_batch_commands_ssh_exception_emits_summary_and_members(monkeypatch):
    ...
    out = execute_batch_commands(
        commands=["a", "b"], business_name="ssh-fail", runtime=runtime
    )
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is False
...
def test_execute_batch_commands_generic_exception_emits_summary_and_members(monkeypatch):
    ...
    out = execute_batch_commands(
        commands=["c", "d"], business_name="exec-fail", runtime=runtime
    )
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is False
...
def test_execute_batch_commands_config_unresolved_emits_summary_and_members(monkeypatch):
    ...
    out = execute_batch_commands(
        commands=["ls", "whoami"], business_name="mybiz", runtime=runtime
    )
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is False
    assert payload.get("error") == "无法解析服务器配置"
    ...
    for m in members:
        assert m.correlation_id == summary_cid
    # error_code = config_unresolved,decision = config_failed
...
def test_get_system_logs_success_even_when_stderr_noisy(monkeypatch):
    ...
    payload = json.loads(out.update["messages"][0].content)
    assert payload.get("success") is True
    # get_system_logs 成功路径仍然成功,即使 stderr 有噪声
    evt = captured[-1]
    assert str(evt.result) == "success"
    assert evt.metadata["exit_code"] == 0
    assert evt.metadata["error_code"] is None
...
def test_blocked_sensitive_command_persists_redacted_via_real_log_service(monkeypatch):
    ...
    async def _lifecycle():
        ...
        # 敏感命令:含 --password=secret123
        execute_command(
            command="mysql --password=secret123 -e 'select 1'",
            business_name="persist-redact",
            runtime=runtime,
        )
    ...
    # 敏感口令 "secret123" 必须已脱敏(由 redact_command 在 _emit_log 前处理)
    assert "secret123" not in blob
    # 但 command_redacted 字段是非空字符串(含占位)
    assert md.get("command_redacted")
    # command_hash 仍对原始命令(脱敏前)做 SHA-256
    from app.shared.utils.log_service import hash_command

    assert md.get("command_hash") == hash_command(
        "mysql --password=secret123 -e 'select 1'"
    )

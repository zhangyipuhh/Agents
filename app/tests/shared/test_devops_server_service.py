# -*- coding:utf-8 -*-
"""
DevOpsServerService 单元测试（2026-07-15 新增；2026-08-03 巡检脚本库改造）

覆盖目标：
    - DevOpsServerService(db, config_path, credential_key,
      inspection_script_service) 的初始化与 singleton 行为
    - preload_all() 从 DB 加载服务器到缓存（含 inspection_script_id 外键）
    - scan_and_upsert() 读取 YAML、规范化、Fernet 加密、upsert 写库、刷新缓存
      （通过 InspectionScriptService.resolve_script_for_server 解析
    ...
    - get_connection_config() 内部解密，绝不外泄密码/IP；
      巡检脚本字段通过 InspectionScriptService 间接获取
    - get_server_detail() 返回 inspection_script_id / inspection_script_name /
      inspection_script_display_name，不返回脚本原文
...
def _make_inspection_script_service() -> MagicMock:
    """构造 InspectionScriptService 替身（含 linux-bash + windows-ps-5.1 默认条目）。

    Returns:
        MagicMock: 替身实例，包含 _cache / resolve_script_for_server /
        get_script_by_id / get_script_by_name / list_scripts 等方法
    """
    ...
    svc.list_scripts.return_value = [
        {"id": 1, "name": "linux-bash", "display_name": "Linux Bash 巡检",
         "platform": "linux", "version": "bash", "inspection_parser": "json",
         "updated_at": "2026-08-03"},
        {"id": 2, "name": "windows-ps-5.1", "display_name": "Windows PS 5.1 巡检",
...
def test_credential_key_invalid_raises_value_error(tmp_yaml):
    ...
    with pytest.raises(ValueError):
        ...
        DevOpsServerService(
            ...
            credential_key="not-a-valid-fernet-key",
        )
...
def test_preload_all_loads_db_rows_into_cache(tmp_yaml):
    ...
    assert svc._cache["alpha"]["inspection_script_id"] == 1
...
def test_scan_and_upsert_returns_public_listing_includes_new_record(tmp_yaml):
    """scan_and_upsert 成功 → 缓存含规范键 + DB 返回真实值。
    ...
    # inspection_script_id 由 InspectionScriptService 默认匹配返回
    assert rec["inspection_script_id"] == 1
...
def test_scan_and_upsert_cache_contains_decryptable_password(tmp_yaml):
    ...
    # 巡检脚本字段通过 InspectionScriptService 间接获取（外部契约不变）
    assert cfg["inspection_script"] == "echo linux"
    assert cfg["inspection_parser"] == "json"
    assert len(cfg["inspection_fields"]) == 1
...
def test_get_connection_config_decrypts_password(tmp_yaml):
    """get_connection_config(business_name) 内部解密并返回完整配置。
    ...
    # 巡检脚本字段由 InspectionScriptService 间接提供
    assert cfg["inspection_script"] == "echo linux"
    assert cfg["inspection_parser"] == "json"
...
def test_get_connection_config_missing_script_id_raises(tmp_yaml):
    """inspection_script_id 为空时抛 ValueError。
    ...
    asyncio.run(svc.preload_all())
    with pytest.raises(ValueError):
        svc.get_connection_config("alpha")
...
# ----------------------------------------------------------------------
# 14. inspection_script_name 解析（2026-08-03 新增）
# ----------------------------------------------------------------------


def test_normalize_inspection_script_name_explicit(tmp_yaml):
    """_normalize_entry 接受 inspection_script_name 显式指定。"""
    from app.shared.utils.devops_server_service import DevOpsServerService
    ...
    assert norm["inspection_script_id"] == 1


def test_normalize_inspection_script_name_default(tmp_yaml):
    """_normalize_entry 不指定 inspection_script_name 时按 server_type 走默认匹配。"""
    from app.shared.utils.devops_server_service import DevOpsServerService
    ...
    assert svc._normalize_entry(linux_entry)["inspection_script_id"] == 1
    assert svc._normalize_entry(win_entry)["inspection_script_id"] == 2


def test_normalize_inspection_script_name_unknown_raises(tmp_yaml):
    """_normalize_entry 显式 inspection_script_name 在脚本库中不存在时抛 ValueError。"""
    from app.shared.utils.devops_server_service import DevOpsServerService
    ...
    entry = {
        ...
        "password": "x",
        "server_type": "linux",
        "inspection_script_name": "no-such-script",
    }
    with pytest.raises(ValueError):
        svc._normalize_entry(entry)


def test_normalize_inspection_script_name_invalid_type_raises(tmp_yaml):
    """_normalize_entry 收到非字符串 inspection_script_name 时抛 ValueError。"""
    from app.shared.utils.devops_server_service import DevOpsServerService
    ...
    entry = {
        ...
        "password": "x",
        "server_type": "linux",
        "inspection_script_name": 12345,
    }
    with pytest.raises(ValueError):
        svc._normalize_entry(entry)


def test_scan_and_upsert_writes_inspection_script_id(tmp_yaml):
    """scan_and_upsert SQL args[9] 是 inspection_script_id（int）。"""
    import asyncio
    from cryptography.fernet import Fernet
    from app.shared.utils.devops_server_service import DevOpsServerService
    ...
    # SQL 含 inspection_script_id
    sql = db.fetchrow.await_args.args[0]
    assert "inspection_script_id" in sql
    # 第 9 个参数就是 inspection_script_id
    args = db.fetchrow.await_args.args
    assert args[9] == 1


def test_preload_loads_inspection_script_id(tmp_yaml):
    """preload_all 加载 DB 行时正确写入 inspection_script_id。"""
    import asyncio
    from app.shared.utils.devops_server_service import DevOpsServerService
    ...
    db.fetch.return_value = [
        ...
        },
        {
            ...
            "blacklist": [],
            "whitelist": [],
            "inspection_script_id": None,  # 缺失 → None
            "created_at": None,
            "updated_at": None,
        },
    ]
    ...
    asyncio.run(svc.preload_all())
    assert svc._cache["alpha"]["inspection_script_id"] == 11
    assert svc._cache["beta"]["inspection_script_id"] is None


# ----------------------------------------------------------------------
# 15. get_server_detail 详情接口（2026-08-03 改造）
# ----------------------------------------------------------------------


def test_get_server_detail_returns_inspection_script_metadata():
    """详情接口：命中时返回 inspection_script_id / inspection_script_name /
    inspection_script_display_name，且不外泄脚本原文与敏感字段。
    """
    from app.shared.utils.devops_server_service import DevOpsServerService

    db = _make_db()
    script_svc = _make_inspection_script_service()
    svc = DevOpsServerService(
        db=db, config_path="unused.yaml",
        credential_key=VALID_FERNET_KEY,
        inspection_script_service=script_svc,
    )
    svc._cache = {
        "alpha": {
            "id": 7,
    ...
    detail = svc.get_server_detail(7)
    assert detail is not None
    assert detail["id"] == 7
    assert detail["business_name"] == "alpha"
    assert detail["whitelist"] == ["ls", "df -h"]
    assert detail["inspection_script_id"] == 1
    assert detail["inspection_script_name"] == "linux-bash"
    assert detail["inspection_script_display_name"] == "Linux Bash 巡检"
    # 安全边界：仍不外泄 ip / port / username / password / blacklist
    ...
    # 脚本原文 / 字段规则不在 detail 里（改走 inspection_script_admin_router）
    assert "inspection_script" not in detail
    assert "inspection_fields" not in detail
    assert "inspection_parser" not in detail


def test_get_server_detail_missing_returns_none():
    """详情接口：未命中时返回 None（不抛 404）。"""
    from app.shared.utils.devops_server_service import DevOpsServerService
...
def test_get_server_detail_with_unregistered_script_id():
    """inspection_script_id 在脚本库中查不到时，detail 返回 name/display_name = None。"""
    from app.shared.utils.devops_server_service import DevOpsServerService
    ...
    svc._cache = {
        "alpha": {
            ...
            "updated_at": "2026-08-03",
            "whitelist": [],
            "inspection_script_id": 999,  # 脚本库中不存在
            "ip": "10.0.0.99",
            "port": 22,
            "username": "root",
            "password_encrypted": b"x",
    ...
    detail = svc.get_server_detail(9)
    assert detail is not None
    assert detail["inspection_script_id"] == 999
    assert detail["inspection_script_name"] is None
    assert detail["inspection_script_display_name"] is None


# ----------------------------------------------------------------------
# 16. get_connection_config 通过 InspectionScriptService 间接获取脚本
# ----------------------------------------------------------------------


def test_get_connection_config_exposes_inspection_fields(tmp_yaml):
    """get_connection_config 返回值 inspection_script / inspection_parser / inspection_fields
    由 InspectionScriptService 间接提供（外部契约不变）。
    """
    import asyncio
    from app.shared.utils.devops_server_service import DevOpsServerService
    ...
    assert len(fields) == 1
    # 注入到脚本侧的契约：list[InspectionFieldRule]

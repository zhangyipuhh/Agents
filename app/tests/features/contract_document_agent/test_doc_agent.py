...
def test_doc_agent_constructor_accepts_base_system_prompt_single_space():
    """
    测试 DocAgent 构造时透传 base_system_prompt=" "（单空格）原样透传（2026-08-20 回归）。

    背景：contract_router.get_doc_agent() 显式传 base_system_prompt=" "（单空格），
    触发三元语义「非空字符串覆盖」分支，等同跳过 BASE_SYSTEM_PROMPT 通用基类。
    用例目的：确保调用方在 DocAgent(checkpointer, store, ..., base_system_prompt=" ") 时
    1) 构造不抛错
    2) 实例字段 self.base_system_prompt == " "
    3) _ensure_agent 构造 DocAgentConfig 时把该值原样传给 config.base_system_prompt

    与 test_doc_agent_constructor_accepts_base_system_prompt 的 "" 用例互为对照。
    """
    from unittest.mock import patch
    from langgraph.store.memory import InMemoryStore
    ...
    assert "config" in captured, "get_agent 未被调用"
    assert captured["config"].base_system_prompt == " ", (
        "DocAgent 必须把 base_system_prompt=' ' 原样透传到 DocAgentConfig.base_system_prompt"
    )
    assert captured["config"].system_prompt == "DOC_SPECIFIC"

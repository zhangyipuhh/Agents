...
# 2026-07-06 改造：检测对象从 request.is_disconnected() 改为 abort_event.is_set()。
...
async def sandbox(  # 2026-06-15: 改 async，支持子智能体停止信号感知
    ...
    ## 2026-07-06 改造：从 is_disconnected 改为 abort_event（主动 abort 通道）

    通过 ``app.core.tools._stop_signal`` 取出当前请求的 abort event（按 session_id 索引）。
    在 ``child_agent.astream()`` 循环中每 ``_STOP_CHECK_INTERVAL`` 个 chunk 检测一次
    ``abort_event.is_set()``，发现客户端调 ``POST /api/agent/{session_id}/abort``
    触发 abort 时立即：

    1. 跳出 astream 循环
    2. 推送 ``tool_stop`` 事件，``data.status = "stopped_by_user"``（前端可识别）
    3. 清理 Docker 容器资源（``middleware.cleanup()``）
    4. 返回 ``Command`` 包含「子智能体已被用户中止」+ ``ToolMessage(tool_call_id)``，
       让 LangGraph state 正确配对 AIMessage.tool_calls → ToolMessage，
       避免下次会话恢复时触发 2013 "tool call result does not follow tool call" 错误

    双保险机制：
    - abort_event：主动 abort（用户点"停止"按钮时由 /abort 路由触发）
    - is_disconnected：兜底（浏览器关闭、网络异常等非主动关闭场景）
    - 两者都触发时任一即可停止子智能体
    ...
    # 2026-07-06 改造：从 ContextVar 取出当前 FastAPI Request（可能为 None）
    ...
    # 2026-07-06 新增：取出 abort_event（按 session_id 索引）
    ...
    # 捕捉"刚点 stop 就进 sandbox"的场景：避免哪怕跑一个 chunk 就立即 break
    # 设计：仅记录日志，不直接置 stopped_by_user=True（由下方主循环统一处理 stopped_by_user 分支）
    if abort_event is not None and abort_event.is_set():
        logger.info(
            "[sandbox] 进入 stream 前已检测到 abort_event，"
    ...
    try:
        ...
        # 2026-07-06 改造：sync stream → async astream（保留原注释）
        # 2026-07-06 增强：检测对象从 is_disconnected 改为 abort_event 优先
        async for chunk in child_agent.astream(
            {"messages": [{"role": "user", "content": prompt}]},
            config=config,
            stream_mode=["updates", "values", "messages"],
        ):
            # 2026-07-06 改造：客户端断开检测（每 N 个 chunk 检测一次）
            # 检测顺序：abort_event（主动 abort 通道）→ is_disconnected（兜底）
            # 任一触发即视为用户停止。
            if len(all_messages) % _STOP_CHECK_INTERVAL == 0:
                # 1. 主动 abort 检查（优先）—— abort_event.set() 由 /abort 路由触发
                if abort_event is not None and abort_event.is_set():
                    logger.info(
                        "[sandbox] abort_event 已 set，停止子智能体。tool_call_id=%s, session_id=%s",
                        tool_call_id, session_id,
                    )
                ...
                if request is not None:
                    try:
                        if await request.is_disconnected():
                            logger.info(
                                "[sandbox] 客户端已断开（兜底），停止子智能体。tool_call_id=%s",
                                tool_call_id,
                            )
                            stopped_by_user = True
                            break

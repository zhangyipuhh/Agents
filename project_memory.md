...
13. **QQ SMTP 出口对陌生域名外发有限制（2026-07-18 实测）**：用我们系统配 `smtp.qq.com` 发到 `zhangyipu@geostar.com.cn` 仍收不到，但用 QQ 邮箱**网页版 HTTP API** 发能收到。Foxmail 也是 SMTP 协议也不行。说明 `smtp.qq.com` 这个 SMTP 出口对陌生域名的外发做了反垃圾限速/拦截，而 HTTP API 不受限。**这不是代码能解决的问题**，需要：① 让 IT 查企业邮箱投递日志；② 测试企业内部发送确认代码是否工作；③ 如确需 QQ SMTP 外发，需向 QQ 申请 IP 白名单或更换为 QQ 企业邮箱（smtp.exmail.qq.com）。
14. **系统代码正确性已最终确认（2026-07-18 用户实测）**：企业邮箱 SMTP（mail.geostar.com.cn）→ 企业内部邮箱能通 → 系统邮件发送代码 100% 正确；之前所有代码修改（RFC 5322 必备头、SSL 兼容、STARTTLS 跳过、证书校验关闭、refused 校验）都是**必要且正确的预防性修复**，覆盖企业邮箱各类协议兼容性场景。**跨域投递问题（企业→QQ / QQ→企业）属 SMTP 出口与反垃圾策略问题，与代码无关**。
...
17. **EHLO local_hostname=cfg.host 保留不改（2026-07-18 排查结论）**：曾怀疑从动态 IP 以 `EHLO smtp.qq.com` 声明是伪造特征导致对方网关静默丢弃，但实测腾讯官方 Foxmail 客户端（EHLO 规范）走 `smtp.qq.com` → geostar 同样失败（见第 13 条），证明分水岭在 QQ SMTP 出口策略而非 EHLO；且改动 `local_hostname` 会让已确认可用的企业邮箱路径承担回归风险，故保持现状。

...
- `web/Agent/src/components/TaskSchedulerManager.vue` 的定时任务卡片为 `article.task-item` 容器，内部由 `.task-select-btn` 负责编辑任务选择、`.task-history-btn` 负责仅图标执行历史入口；执行历史通过 `historyScheduleId` 单卡片展开，复用 `fetchTaskRuns(scheduleId, 50)`，不改变后端接口或数据库结构；`TaskSchedulerManager.spec.js` 覆盖图标按钮无文字、可访问名称、单卡片展开/收起、事件隔离和局部错误展示。

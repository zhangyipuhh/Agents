...
| 分片 | 内容 | 关键词 | 更新时间 |
|---|---|---|---|
| [架构与目录](memory/architecture.md) | 项目概述、技术栈、数据目录约定、项目架构、配置归属隔离、项目文件夹方案、环境变量、**等保三级安全编码规范** | 架构、技术栈、目录、路径、环境变量、OwnershipScope、等保三级、安全规范 | 2026-08-08 |
...
| [认证与会话控制](memory/auth.md) | 双 Token 认证、浏览器登录 TOTP 两阶段认证、管理员强制/普通用户可选、challenge 防重放与失败锁定、amr、Portal 子 token、聊天并发控制、HITL、提示词与上下文、MFA 默认 issuer=AIOps（可由 MFA_ISSUER 覆盖）、**`user_mfa_totp.enabled_at` naive TIMESTAMP 列写入约束（2026-08-08 修复 offset-aware 误传 bug）**、**登录失败计数与账号锁定（`failed_login_count` / `locked_until`，路由双重保险，2026-08-08 修复 DatabasePool.fetchval 缺失导致累计链路静默失效）** | 认证、MFA、TOTP、双因素、Token、amr、challenge、恢复码、锁定、并发、HITL、提示词、上下文、issuer、AIOps、MFA_ISSUER、enabled_at、naive、datetime、replace(tzinfo=None)、offset-aware、failed_login_count、locked_until、max_attempts、lockout_seconds、record_failed_login、get_login_lock_state、clear_login_lock、DatabasePool.fetchval、CTE、fetchrow、双重保险、logger.exception、except Exception: pass | 2026-08-08 |

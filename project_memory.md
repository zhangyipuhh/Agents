...
| [认证与会话控制](memory/auth.md) | 双 Token 认证、浏览器登录 TOTP 两阶段认证、管理员强制/普通用户可选、challenge 防重放与失败锁定、amr、Portal 子 token、聊天并发控制、HITL、提示词与上下文、MFA 默认 issuer=AIOps（可由 MFA_ISSUER 覆盖）、**`user_mfa_totp.enabled_at` naive TIMESTAMP 列写入约束（2026-08-08 修复 offset-aware 误传 bug）**、**登录失败计数与账号锁定（`failed_login_count` / `locked_until`，路由双重保险，2026-08-08 修复 DatabasePool.fetchval 缺失导致累计链路静默失效）**、**Access Token 提取顺序（Bearer 优先 → HttpOnly Cookie 兜底，`request.state.auth_via` 标记来源；Cookie 鉴权写请求中间件强制要求 `X-Requested-With: XMLHttpRequest` 头做 CSRF 二次防线）**、**refresh 成功轮换 access_token HttpOnly Cookie 且 validate 支持 Cookie 兜底（2026-08-08）**、**Admin 编辑用户不允许改密码（`UserUpdateRequest` 不含 password，密码修改走 `PUT /api/users/{user_id}/password` 且仅本人需 old_password）** | 认证、MFA、TOTP、双因素、Token、amr、challenge、恢复码、锁定、并发、HITL、提示词、上下文、issuer、AIOps、MFA_ISSUER、enabled_at、naive、datetime、replace(tzinfo=None)、offset-aware、failed_login_count、locked_until、max_attempts、lockout_seconds、record_failed_login、get_login_lock_state、clear_login_lock、DatabasePool.fetchval、CTE、fetchrow、双重保险、logger.exception、except Exception: pass、Bearer、HttpOnly Cookie、access_token、auth_via、CSRF、X-Requested-With、XMLHttpRequest、二次防线、refresh、validate、access_token Cookie、X-Requested-With CSRF、HttpOnly Cookie 鉴权、AuthCookieSettings、admin 不改密码、UserUpdateRequest 不含 password、old_password 仅本人 | 2026-08-08 |

...
| 分片 | 内容 | 关键词 | 更新时间 |
|---|---|---|---|
| [架构与目录](memory/architecture.md) | 项目概述、技术栈、数据目录约定、项目架构、配置归属隔离、项目文件夹方案、环境变量、**`AuthCookieSettings` 认证 Cookie 配置（HttpOnly Cookie 迁移基础设施，env 前缀 `AUTH_COOKIE_`）**、**等保三级安全编码规范**、**本地 HTTPS 测试 nginx/（mkcert + 8443/8080，与 web/Agent/nginx.conf 同步 + TLS/HSTS 等保三级加固 + HTML 入口禁缓存 `location ~* \.html$`）** | 架构、技术栈、目录、路径、环境变量、OwnershipScope、等保三级、安全规范、AuthCookieSettings、HttpOnly Cookie、AUTH_COOKIE_、nginx、HTTPS、mkcert、8443、8080、HSTS、TLSv1.2、TLSv1.3、HTML 禁缓存、no-cache、.html$ | 2026-08-09 |

...
- **Admin 管理**（2026-06-23 / 2026-06-24 新增）：
  ...
  - `AgentManager.vue`（**2026-06-24 新增**）：智能体管理 Tab 内容；左侧智能体列表 + 右侧三组可编辑表格（AgentConfig 字段 / State 字段 / Context 字段）；支持完整 CRUD：
    ...
    - **保存策略（2026-06-24 修复）**：`modified` 字段改用 `PUT /config-schema/field` 直接覆盖，避免旧版"先删后加"导致的数据丢失；`delete` 失败时记录具体字段名并继续处理其他变更，错误信息汇总展示；失败时保留 `pendingChanges` 不自动清空
    - **删除智能体**：含确认弹窗（保留历史会话）
...
### InputBox 智能体快速选择（2026-06-24 新增）

...
| 分片 | 内容 | 关键词 | 更新时间 |
|---|---|---|---|
| [架构与目录](memory/architecture.md) | 项目概述、技术栈、数据目录约定、项目架构、配置归属隔离、项目文件夹方案、环境变量、**`AuthCookieSettings` 认证 Cookie 配置（HttpOnly Cookie 迁移基础设施，env 前缀 `AUTH_COOKIE_`）**、**等保三级安全编码规范**、**本地 HTTPS 测试 nginx/（mkcert + 8443/8080，与 web/Agent/nginx.conf 同步 + TLS/HSTS 等保三级加固 + HTML 入口禁缓存 `location ~* \.html$`）**、**HTTPS 模块加载踩坑记录（2026-08-09：Vite build 的 crossorigin module 在 HTTPS 下需 ACAO 头；nginx 子级 add_header 覆盖父级；Chromium CORS 失败有 network-level negative cache 必须硬刷新）** | 架构、技术栈、目录、路径、环境变量、OwnershipScope、等保三级、安全规范、AuthCookieSettings、HttpOnly Cookie、AUTH_COOKIE_、nginx、HTTPS、mkcert、8443、8080、HSTS、TLSv1.2、TLSv1.3、HTML 禁缓存、no-cache、.html$、双后端进程、8001、9001、VITE_API_TARGET、checkpointer 回退、MemorySaver、AsyncPostgresSaver、fail-loud、crossorigin、ACAO、CORS、Access-Control-Allow-Origin、模块加载、DevTools Console、硬刷新、Ctrl+Shift+R、network-level negative cache、HTTP 与 HTTPS 差异 | 2026-08-09 |
| [安全合规](memory/security-compliance.md) | 等保三级（GB/T 22239-2019）应用层面合规检查清单，覆盖身份鉴别、访问控制、安全审计、入侵防范、恶意代码防范、软件容错、数据完整性、数据保密性、数据备份恢复、剩余信息保护、个人信息保护及 HTTPS 快速启动清单 | 等保三级、安全合规、合规检查、GB/T 22239-2019、身份鉴别、访问控制、安全审计、入侵防范、数据保密性、个人信息保护 | 2026-08-07 |

...
const NAV_MENU_METADATA = {
  ...
  // - 后端 MENU_CATALOG 注册为 level=1,icon_key='settings',sort_order=0（2026-09-15 上移到顶部第一位）
  // - 路由 /api/admin/system-settings/*,内部含 6 孙 Tab
  // 2026-09-15 调整：键位置从对象末尾移至首位（与后端 sort_order=0 对齐），保证 Object.values() 渲染顺序为「基本设置」在前
  ...
  // 2026-07-23 调整：「权限管理」键位置上移到「用户管理」正下方，与后端 MENU_CATALOG sort_order 对齐。
  ...
  // - 后端 MENU_CATALOG 注册为 level=1，icon_key='message'，sort_order=9（2026-09-15 基本设置上移后顺次 +1）
...
/* 设置子 Tab 高度链 wrapper：
   ...
   overflow-y:auto 是安全网：未自滚动填充的组件（如 EmailSettingsManager）
   内容超高时由 wrapper 滚动，避免被 .dialog-body-horizontal 的 overflow:hidden 裁剪；
   内部已自滚动的组件（TaskSchedulerManager）正常填满时不会触发该滚动。 */
.tab-fill-wrapper {
  display: flex;
  flex: 1;
  min-height: 0;

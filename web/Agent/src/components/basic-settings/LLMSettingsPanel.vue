...
const llmFields = [
  { name: 'model_type', label: '模型类型', type: 'str', description: 'openai / anthropic / ollama / google / deepseek' },
  ...
  { name: 'parallel_tool_calls', label: '并行工具调用', type: 'str', placeholder: 'none / true / false', description: 'none = 走 LLM 全局;true/false 强制覆盖' },
...
const visionFields = [
  { name: 'model_type_vision', label: '模型类型', type: 'str', description: 'openai / anthropic / google 等' },
...
const samplingFields = [
  { name: 'mcp_sampling_model_type', label: '模型类型', type: 'str', description: 'MCP Sampling 调用的模型类型' },
...
const contractFields = [
  { name: 'model_type', label: '模型类型', type: 'str', description: '合同路由独立模型类型' },
  ...
  { name: 'parallel_tool_calls', label: '并行工具调用', type: 'str', description: 'Ollama 默认 true 可能与 file_chunk_read_progress 单 superstep 写冲突,推荐 false' },
];
</script>
...
describe('TaskSchedulerManager 组件', () => {
  ...
  it('test_default_tab_is_edit_task 默认 Tab 是「编辑任务」', async () => {
    ...
    // 2026-07-24 新增：切到「服务器管理」Tab 后，仅 servers 面板存在
    ...
    expect(wrapper.find('[data-testid="panel-script"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid="panel-api"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid="panel-servers"]').exists()).toBe(true)
  })
  ...
  it('test_server_updated_at_format_iso_to_YYYY_MM_DD_HH_MM_SS 最近同步时间显示为绝对时间', async () => {
    const wrapper = mount(TaskSchedulerManager, { props: { isAdmin: true } })
    await flushPromises()
    await wrapper.findAll('[role="tab"]')[1].trigger('click')
    await flushPromises()

    const updatedAtText = wrapper.find('[data-testid="server-row-1"]').findAll('td')[2].text().trim()
    expect(updatedAtText).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/)
    expect(updatedAtText).not.toContain('T')
    expect(updatedAtText).not.toContain('.')
...
/**
 * 巡检脚本库 Tab 端到端契约(2026-08-04 新增;2026-09-16 移除扫描按钮)
 *
 * 覆盖:
 *  - admin 可见「巡检脚本库」Tab 且排在第 6 个
 *  - 普通用户未授权时 Tab 不渲染
 *  - 切换到「巡检脚本库」Tab 后渲染左右分栏
 *  - 2026-09-16 移除:扫描按钮 / library-scan-btn / library-scan-summary 均不应存在
...
describe('TaskSchedulerManager 巡检脚本库 Tab（2026-08-04 新增）', () => {
  ...
  it('test_library_scan_button_removed 2026-09-16:扫描按钮已移除', async () => {
    const wrapper = mount(TaskSchedulerManager, { props: { isAdmin: true } })
    await flushPromises()
    const libTab = wrapper.findAll('[role="tab"]').find((t) => t.text() === '巡检脚本库')
    await libTab.trigger('click')
    await flushPromises()
    // 扫描按钮与 summary DOM 不应存在
    expect(wrapper.find('[data-testid="library-scan-btn"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid="library-scan-summary"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid="library-scan-status"]').exists()).toBe(false)
    // 任何 /scan 调用都不应被发起
    const scanCalls = global.fetch.mock.calls.filter(
      ([url, opts]) => typeof url === 'string' && url.includes('/inspection-scripts/scan')
    )
  ...
  it('test_node_click_opens_editor_and_save_triggers_put 选中节点后保存触发 PUT', async () => {
    ...
    // 保存成功后应展示 library-save-status 提示
    expect(wrapper.find('[data-testid="library-save-status"]').exists()).toBe(true)
    wrapper.unmount()
  })
})
...
describe('TaskSchedulerManager 内部滚动契约（防溢出）', () => {
  ...
  it('test_business_panels_scroll_internally 业务面板（编辑/扫描）内部滚动', () => {
    // 2026-07-24 调整：task-panel-servers 也加入 :not 排除（与 api panel 行为对齐）
    const body = styleBlock('.task-detail > section[role="tabpanel"]:not(.task-panel-api):not(.task-panel-servers)')

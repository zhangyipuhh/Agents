...
// === Tab 切换状态 ===
const activeTab = ref('basic') // 'basic'（基本信息） | 'config'（配置字段） | 'tools'（工具绑定） | 'skills'（Skill 绑定）
...
// === Skill 绑定相关状态 ===
const availableSkills = ref([])            // 当前 agent 可绑定的 skill 列表（fetchAgentAvailableSkills 返回）
const agentSkillBindings = ref([])         // 当前 agent 已保存的 skill 绑定（getAgentSkillBindings 返回）
const localSelectedSkillBindings = ref({}) // 本地勾选状态：key = skill_name -> { enabled, sort_order }
const isLoadingSkills = ref(false)         // Skill 绑定 Tab 加载状态
const skillBindingsError = ref('')         // Skill 绑定 Tab 错误信息
const skillBindingsSavedTip = ref('')      // Skill 绑定保存成功提示

// === Skill 绑定：分组折叠状态 ===
const expandedSkillGroups = ref(new Set()) // 已展开的 skill 分类集合
...
/**
 * 切换 skill 分组的展开/折叠状态
 * @param {string} category - skill 分类名称
 */
function toggleSkillGroup(category) {
...
async function selectAgent(name) {
  ...
  // 切换 agent 时重置 skill 绑定本地状态
  agentSkillBindings.value = []
  localSelectedSkillBindings.value = {}
  skillBindingsError.value = ''
  skillBindingsSavedTip.value = ''
  // 记录切换前是否在工具绑定 / Skill 绑定 Tab，用于决定是否需要重新加载绑定
  const wasOnToolsTab = activeTab.value === 'tools'
  const wasOnSkillsTab = activeTab.value === 'skills'
  try {
    ...
    // 若当前停留在 Skill 绑定 Tab，切换 agent 后立即重新加载该 agent 的 skill 绑定
    if (wasOnSkillsTab) {
      await loadAvailableSkills()
      await loadAgentSkillBindings()
    }
  } catch (err) {
 ...
 * @param {string} tab - 目标 Tab 名称（basic / config / tools / skills）
 * @returns {Promise<void>} 无返回值
 */
async function switchTab(tab) {
  ...
  if (tab === 'skills' && selectedAgentName.value) {
    await onSwitchToSkillsTab()
  }
}
...
/**
 * 判断指定 skill 是否在本地勾选集合中
 * @param {string} skillName - skill 名称
 * @returns {boolean} 是否勾选
 */
function isSkillSelected(skillName) {
  return !!localSelectedSkillBindings.value[skillName]
}

/**
 * 按 category 分组展示所有可用 skill
...
const groupedSkills = computed(() => {
  ...
  for (const skill of availableSkills.value) {
    ...
    groups[category].push({
      ...
      sourceEnabled: skill.enabled !== false,
    })
  }
 ...
 * 当前勾选的 skill 绑定数量
 */
const selectedSkillBindingsCount = computed(() => {
  return Object.keys(localSelectedSkillBindings.value).length
})

/**
 * skill 绑定是否有未保存的修改（对比本地勾选与服务端已保存的绑定）
 * @returns {boolean} 是否存在差异
 */
const hasSkillBindingChanges = computed(() => {
  const savedSet = new Set()
  for (const b of agentSkillBindings.value) {
    if (b.enabled) savedSet.add(b.skill_name)
  }
  const localSet = new Set(Object.keys(localSelectedSkillBindings.value))
  if (savedSet.size !== localSet.size) return true
  for (const k of localSet) {
    if (!savedSet.has(k)) return true
  }
...
/**
 * 加载当前 agent 可用的 skill 列表（fetchAgentAvailableSkills）
 * @returns {Promise<void>} 无返回值；失败时设置 skillBindingsError
 */
async function loadAvailableSkills() {
  if (!selectedAgentName.value) return
  skillBindingsError.value = ''
  try {
    const data = await fetchAgentAvailableSkills(selectedAgentName.value)
    availableSkills.value = Array.isArray(data.skills) ? data.skills : []
  } catch (err) {
    skillBindingsError.value = err.message || '加载可用 skill 列表失败'
    availableSkills.value = []
  }
}

/**
 * 加载当前选中 agent 的 skill 绑定，并同步本地勾选状态
 * @returns {Promise<void>} 无返回值；失败时设置 skillBindingsError
 */
async function loadAgentSkillBindings() {
  if (!selectedAgentName.value) return
  skillBindingsError.value = ''
  try {
    const data = await getAgentSkillBindings(selectedAgentName.value)
    const bindings = data.skill_bindings || []
    agentSkillBindings.value = bindings
    // 同步本地勾选状态：仅 enabled=true 的 skill 被勾选
    const selected = {}
    for (const b of bindings) {
      if (b.enabled) {
        selected[b.skill_name] = { enabled: true, sort_order: b.sort_order ?? 0 }
      }
    }
    localSelectedSkillBindings.value = selected
  } catch (err) {
    skillBindingsError.value = err.message || '加载 skill 绑定失败'
    agentSkillBindings.value = []
    localSelectedSkillBindings.value = {}
  }
}
...
/**
 * 切换到 Skill 绑定 Tab 时触发加载（可用 skill 列表 + 当前 agent 绑定）
 * @returns {Promise<void>} 无返回值
 */
async function onSwitchToSkillsTab() {
...
function toggleSkillSelection(skillName) {
  if (localSelectedSkillBindings.value[skillName]) {
    const next = { ...localSelectedSkillBindings.value }
    delete next[skillName]
    localSelectedSkillBindings.value = next
  } else {
    localSelectedSkillBindings.value = {
      ...localSelectedSkillBindings.value,
      [skillName]: { enabled: true, sort_order: 0 },
    }
  }
}
...
/**
 * 保存 skill 绑定到后端
 * 构造 bindings 数组（仅勾选的 skill，sort_order 按分组顺序生成），
 * 调用 updateAgentSkillBindings；成功后重新加载绑定状态并显示提示
 * @returns {Promise<void>} 无返回值；失败时设置 skillBindingsError
 */
async function saveSkillBindings() {
  ...
  skillBindingsError.value = ''
  skillBindingsSavedTip.value = ''
  // 构造 bindings：所有勾选的 skill，sort_order 按分组顺序生成
  const bindings = []
  let sortOrder = 0
  ...
  try {
    await updateAgentSkillBindings(selectedAgentName.value, bindings)
    // 保存成功后重新加载绑定状态，保持本地与服务端一致
    await loadAgentSkillBindings()
    skillBindingsSavedTip.value = 'Skill 绑定保存成功'
    // 3 秒后自动清除提示
    setTimeout(() => { skillBindingsSavedTip.value = '' }, 3000)
  } catch (err) {
    skillBindingsError.value = err.message || '保存 skill 绑定失败'
  }
}
...
onMounted(async () => {
  ...
  if (!props.isAdmin) {
    console.warn('[AgentManager] 非 admin 用户挂载本组件，已跳过数据加载（防御性兜底）')
    return
  }
      ...
      <main class="agent-main">
        ...
        <div v-else>
          ...
          <div v-if="activeTab === 'skills'" class="tab-content">
            <!-- 错误提示 -->
            <div v-if="skillBindingsError" class="error-banner">{{ skillBindingsError }}</div>
            <!-- 保存成功提示 -->
            <div v-if="skillBindingsSavedTip" class="success-banner">{{ skillBindingsSavedTip }}</div>

            <div v-if="isLoadingSkills" class="loading">加载 skill 列表中...</div>
            <div v-else>
              <!-- Skill 绑定说明 + 保存按钮 -->
              <div class="skills-toolbar">
                <div class="skills-summary">
                  已勾选 <strong>{{ selectedSkillBindingsCount }}</strong> 个 skill
                  <span v-if="hasSkillBindingChanges" class="badge-pending">有未保存的修改</span>
                </div>
                <div class="skills-actions">
                  <button
                    class="btn-primary"
                    :disabled="!hasSkillBindingChanges"
                    @click="saveSkillBindings"
                  >
                    保存 skill 绑定
                  </button>
                </div>
              ...
              <div v-else class="skills-groups">
                ...
                >
                  <div
                    ...
                    @click="toggleSkillGroup(group.category)"
                  >
                    <span
                      class="skill-group-arrow"
                      :class="{ expanded: expandedSkillGroups.has(group.category) }"
                    >▶</span>
                    <h5 class="skill-group-title">{{ group.category }}</h5>
                    <span class="skill-group-count">{{ group.skills.length }}</span>
                  </div>
                  <ul v-show="expandedSkillGroups.has(group.category)" class="skill-list">
                    <li
                    ...
                    >
                      <label class="skill-checkbox">
                        <input
                          type="checkbox"
                          :checked="isSkillSelected(skill.skill_name)"
                          @change="toggleSkillSelection(skill.skill_name)"
                        />
                        <span class="skill-name">{{ skill.display_name }}</span>
                        <span v-if="!skill.sourceEnabled" class="skill-disabled-hint">
                          （来源已禁用）
                        </span>
                      </label>
              ...
              <!-- 底部保存按钮（与顶部一致，方便长列表操作） -->
              <div v-if="hasSkillBindingChanges" class="changes-summary">
                <span>skill 绑定有未保存的修改</span>
                <div>
                  <button class="btn-primary" @click="saveSkillBindings">保存 skill 绑定</button>
                </div>
              </div>
            </div>
          </div><!-- /Skill 绑定 Tab -->

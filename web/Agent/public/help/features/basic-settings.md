...
admin 点击左下角头像 → 「管理后台」→「基本设置」。页面顶部提示「本页配置修改后需重启服务生效」，每组配置卡片右上角有「重置默认」可一键恢复出厂值。

#### 字段标注约定
- 输入框右上角灰色标签为配置项键名（如 `model_type` / `model_api_key`），与 `.env` 字段一一对应
- `secret` 标记的字段为敏感字段：Fernet 加密存储，**留空表示不修改原值**
- 开关类字段显示「当前值: true / false」，与 DB 实际值同步

...
- `app/routers/api_config_router.py`：`/api/admin/api-configs` 端点授权契约（2026-07-26 调整，2026-09-12 渗透整改加固）：
  - 只读 `GET /tree`：`Depends(require_admin_or_any_menu_acl('task-scheduler.api-config', 'task-scheduler.scheduled'))`（admin 直接放行；普通用户必须命中任一子菜单 ACL；定时任务表单 api_list 控件候选依赖本端点）；OwnershipScope 按归属过滤（普通用户仅见自己的接口节点）；
...
| ├ POST /chat                       |                        | 合同审批聊天（HtAgent 非流式，受 `chat_concurrency_dependency` 并发控制）；**第三方接入标准**：登录态用户调 `POST /api/auth/issue-portal-refresh-token` 取 portal token → `POST /api/auth/refresh`（`X-Refresh-Token` 头）换 access_token → `POST /api/session/create` 取 session_id → `POST /api/contract/chat`（Bearer + `X-Session-ID`）；详细 curl/fetch 样例见 `app/features/contract_host_agent/THIRD_PARTY_INTEGRATION.md`；**2026-08-20**：`get_ht_agent()` 显式注入 token 三参数（`max_tokens=64000` / `max_tokens_before_summary=50000` / `max_summary_tokens=4000`），假设模型最大上下文 64k；常量定义在文件顶部 `HT_AGENT_MAX_TOKENS` / `HT_AGENT_MAX_TOKENS_BEFORE_SUMMARY` / `HT_AGENT_MAX_SUMMARY_TOKENS`，不走 `.env` / `ContractLLMSettings` |
| ├ POST /doc_chat                   |                        | 文档处理聊天（DocAgent 非流式，受 `chat_concurrency_dependency` 并发控制）；**2026-08-20**：`get_doc_agent()` 延迟初始化工厂显式注入 token 三参数（`max_tokens=64000` / `max_tokens_before_summary=50000` / `max_summary_tokens=4000`），假设模型最大上下文 64k；常量定义在文件顶部 `DOC_AGENT_MAX_TOKENS` / `DOC_AGENT_MAX_TOKENS_BEFORE_SUMMARY` / `DOC_AGENT_MAX_SUMMARY_TOKENS`，不走 `.env` / `ContractLLMSettings`                                         |
| ├ POST /approval_chat              |                        | 审批处理聊天（ApprovalAgent 非流式，受 `chat_concurrency_dependency` 并发控制）；**2026-08-19**：`get_ht_agent()` / `get_doc_agent()` / `get_approval_agent()` 三个延迟初始化工厂默认传 `base_system_prompt=""`，跳过 `app.core.prompts.BASE_SYSTEM_PROMPT`（通用基类规则），仅保留各 Agent 自带的 `DEFAULT_SYSTEM_PROMPT` 约束角色风格；**2026-08-20**：`get_approval_agent()` 显式注入 token 三参数（`max_tokens=64000` / `max_tokens_before_summary=50000` / `max_summary_tokens=4000`），常量定义在文件顶部 `APPROVAL_AGENT_*`，不走配置中心                                       |
...
| 端点 | 权限 | 说明 |
|---|---|---|
| `GET /api/admin/scripts` | `Depends(require_admin_or_menu_acl('task-scheduler.scheduled'))` | admin 直接放行；普通用户需持有 `task-scheduler.scheduled` ACL 才能列出已注册脚本（白名单字段，不暴露源码） |

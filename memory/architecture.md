...
- `app/core/services/system_config_service.py`：`SystemConfigService(pool, settings, log_service)` — `seed_from_settings`（空表 seed）/ `load_all`（DB → settings 单例覆盖 + 解密）/ `get_group`（脱敏）/ `list_groups`（按 tab 分组）/ `update_group`（**校验前先 `_decrypt_sensitive` 解密 merged 中的密文字段再喂 pydantic**（2026-09-14 修复）+ 加密 + `****`/`""` 精确匹配保持原值 + 审计）/ `reset_group`（pydantic default）

  **修复历史（2026-09-14）**：原 `update_group` 直接把 DB 现存的 `fernet:gAAA...` 密文当明文送 pydantic 校验器，导致 `MfaSettings.validate_secret_key` 看到 base64 末尾 padding 不合法就抛 `Incorrect padding` ValueError。用户进 UI 不改敏感字段（前端正确地不传 payload）也会报错。修复：**先解密 merged → 再校验 → 再加密落库**。同时前端 `GroupFormSection` 引入 `sensitivePlaceholders` ref 与 formData 解耦：load 时敏感字段只写占位符到 `sensitivePlaceholders`、不写 formData；input value 始终为空；placeholder 显示「当前值: ****abcd | 留空保持不变」；handleSave 时敏感字段空串/null 不传 payload。后端 `update_group` 的 `****` 判断改为 `v in ("****", "")` 精确匹配（防御用户真实输入 `****xxx`）。

...
- **数据归属上下文注入（2026-08-17 新增）**：`AgentContext.ownership_scope`（字典形态）由 `agent_router.chat` 用 `request.state.user_id` + `request.state.role` 强制覆盖注入到 `context_overrides`，供数据层隔离类工具（如 `InspectionQueryTools.query_inspection_records`）按当前用户角色过滤数据。客户端无法伪造（与 `log_user_id` / `log_ip` 同款覆盖策略）；工具侧 `_resolve_scope` 还原为 `OwnershipScope` 实例（支持 dict / 已构造实例 / 缺失 → system_scope 三层兜底）。**服务实例**（如 `ServerInspectionRecordService`）则走类级单例（lifespan `set_instance` + 工具 `get_instance`），与 `DevOpsServerService` 同款契约，避免污染 `agent_router.chat` 通用通道。
...
- `app/core/services/system_config_service.py`：`SystemConfigService(pool, settings, log_service)` — `seed_from_settings`（空表 seed）/ `load_all`（DB → settings 单例覆盖 + 解密）/ `get_group`（脱敏）/ `list_groups`（按 tab 分组）/ `update_group`（**partial 校验 + partial merge（2026-09-14 修复）**：只校验用户传入的字段、用 `model_dump(include=...)` 避免 pydantic 默认值污染、敏感字段 `****`/`""` 精确匹配从 payload 移除 / 加密 / 审计）/ `reset_group`（pydantic default）

  **修复历史（2026-09-14 三轮）**：
  - **轮 1（前端口）**：`GroupFormSection` 引入 `sensitivePlaceholders` ref 与 formData 解耦；load 时敏感字段只写占位符到 `sensitivePlaceholders`、不写 formData；input value 始终为空；placeholder 显示「当前值: ****abcd | 留空保持不变」；handleSave 时敏感字段空串/null 不传 payload（"不改 = 不传"）。
  - **轮 2（后端口 - `****` 误判）**：`update_group` 的 `v.startswith("****")` 改为 `v in ("****", "")` 精确匹配（防御用户真实输入 `****xxx`）。
  - **轮 3（后端口 - partial 校验根治）**：`update_group` 重写为 partial 更新契约 — payload 只含用户实际改动字段；`_validate_config` 用 `model_dump(include=list(config.keys()))` 只返回用户传入字段（避免 pydantic 默认值污染）；merged 用 `merged.update(validated)` 仅覆盖用户传入字段；未传字段保持 DB 原值（包括敏感字段 fernet: 密文保持原状不重新加密）。**关键洞察**："用户没传的字段根本不需要校验"——原实现要求传入完整 config 是错的设计，校验器看到 DB 现存的 fernet: 密文当然报错。
...
- **证书**：`nginx/conf/certs/localhost.pem` / `localhost-key.pem`，由 `nginx/mkcert.exe` 签发
  ...
  - SAN：`localhost` / `127.0.0.1` / `::1` / `10.20.8.73` / **`172.26.160.50`** / **`124.95.179.91`** / **`10.1.114.190`**（2026-08-12 新增 LAN IP `10.20.8.73`；**2026-08-16 追加 LAN IP `172.26.160.50` 与公网 IP `124.95.179.91`，便于 Docker 容器 / 远程办公机通过 `https://172.26.160.50:8443/` 与 `https://124.95.179.91:8443/` 访问**；**2026-08-18 追加 LAN IP `10.1.114.190`，便于其他机器通过 `https://10.1.114.190:8443/` 访问**），有效期 2 年（到 2028-11-18，2026-08-18 重签）
  - 重新签发命令：`cd .\nginx && .\mkcert.exe -cert-file .\conf\certs\localhost.pem -key-file .\conf\certs\localhost-key.pem localhost 127.0.0.1 ::1 10.20.8.73 172.26.160.50 124.95.179.91 10.1.114.190`（**必须用 `-cert-file` / `-key-file` 覆盖默认文件名**，否则 mkcert 会生成 `localhost+5.pem` 导致 nginx / docker-compose 引用断裂；**mkcert 不会保留默认值，必须显式列出所有 SAN**，否则重签后 CN 也会变成第一个参数，旧 `localhost`/`127.0.0.1`/`::1` 会消失并导致本地访问立即报证书名称不匹配）；签发后需同步到 `.\web\Agent\certs\`（Docker volume bind 注入）
  - mkcert 已注册到 Windows 受信根（`mkcert -install`），浏览器访问 `https://localhost:8443` / `https://10.20.8.73:8443` / `https://172.26.160.50:8443` / `https://124.95.179.91:8443` / `https://10.1.114.190:8443` 显示绿锁无警告（**公网 IP 客户端需手动导入 `nginx/rootCA.pem` 到受信根**，否则互联网用户浏览器会拒绝）

...
- **证书**：`nginx/conf/certs/localhost.pem` / `localhost-key.pem`，由 `nginx/mkcert.exe` 签发
  ...
  - SAN：`localhost` / `127.0.0.1` / `::1` / `10.20.8.73` / **`172.26.160.50`** / **`124.95.179.91`**（2026-08-12 新增 LAN IP `10.20.8.73`；**2026-08-16 追加 LAN IP `172.26.160.50` 与公网 IP `124.95.179.91`，便于 Docker 容器 / 远程办公机通过 `https://172.26.160.50:8443/` 与 `https://124.95.179.91:8443/` 访问**），有效期 2 年（到 2028-11-16，2026-08-16 重签）
  - 重新签发命令：`cd .\nginx && .\mkcert.exe -cert-file .\conf\certs\localhost.pem -key-file .\conf\certs\localhost-key.pem localhost 127.0.0.1 ::1 10.20.8.73 172.26.160.50 124.95.179.91`（**必须用 `-cert-file` / `-key-file` 覆盖默认文件名**，否则 mkcert 会生成 `localhost+5.pem` 导致 nginx / docker-compose 引用断裂；**mkcert 不会保留默认值，必须显式列出所有 SAN**，否则重签后 CN 也会变成第一个参数，旧 `localhost`/`127.0.0.1`/`::1` 会消失并导致本地访问立即报证书名称不匹配）；签发后需同步到 `.\web\Agent\certs\`（Docker volume bind 注入）
  - mkcert 已注册到 Windows 受信根（`mkcert -install`），浏览器访问 `https://localhost:8443` / `https://10.20.8.73:8443` / `https://172.26.160.50:8443` / `https://124.95.179.91:8443` 显示绿锁无警告（**公网 IP 客户端需手动导入 `nginx/rootCA.pem` 到受信根**，否则互联网用户浏览器会拒绝）

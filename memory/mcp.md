...
- `mcp` 主项目锁 **`==1.27.2`**（`app/requirements.txt:124`），`mcpClient` 子目录独立锁 **`>=1.24,<2.0`**（`mcpClient/pyproject.toml`，commit `7d461dd` 2026-09-08 一并校准）
- `langchain-mcp-adapters` 锁 **`==0.3.2`**（`app/requirements.txt:97`，从 0.2.1 升级而来）

**硬约束**：`mcp` 必须落在 **`>=1.27,<2`** 区间（`mcpClient` 子目录 ≥1.24 已自动覆盖 1.27）。

**历史版本契约（2026-09-07 锁定 / 2026-09-08 回退）**：

- 2026-09-07：`mcp>=1.28,<2` + `langchain-mcp-adapters==0.2.1`，原因：`0.2.1` 依赖 `mcp<2` 的 `streamable_http_client` API（1.28 引入）
- 2026-09-08：commit `7d461dd` 把 `mcp` 从 `2.0.0` 回退到 `1.27.2`，`mcpClient/pyproject.toml` 改为 `>=1.24,<2.0`，但 **未同步更新测试断言与本章节**（漂移直至 2026-09-11 conda 同步后装出 1.27.2 测试才暴露）

**`langchain-mcp-adapters==0.3.2` 对 mcp 版本的新约束**：

- 0.3.x 已不强制依赖 mcp≥1.28 的 `streamable_http_client`（0.3.0 起适配更宽的 1.x 区间），所以 mcp 1.27.2 可与 0.3.2 兼容
- **mcp 2.x 仍被拦截**：`major == 1` 主断言守住 mcp 2.x，`mcp.types` 分层改写 + `client.experimental` 顶层 import 抛 `ImportError` 的破坏性变更未变
...
**回归保护**：`app/tests/core/tools/test_mcp_registry_runtime.py` 2 条用例守卫契约：
...
- `test_mcp_version_pinned_to_1x`（2026-09-11 校准）：通过 `importlib.metadata.version("mcp")` 断言 `major == 1` 且 `>= 1.27`，对齐 `app/requirements.txt:124` 的 `mcp==1.27.2` 锁定（原断言 `>=1.28` 已与 requirements 长期漂移并在本轮 conda 同步后失败）。**主断言 `major == 1` 仍是拦截 mcp 2.x 的最终防线**

**requirements.txt 位置**：

- `app/requirements.txt`（主项目）: `mcp==1.27.2`
- `mcpClient/pyproject.toml`（mcpClient 子目录独立 install）: `mcp>=1.24,<2.0`

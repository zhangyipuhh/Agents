...
### 巡检元信息表（2026-08-16 简化至 5 行）

- **当前形态**：`app/scripts/ops/ops_report.py::_server_meta_rows` 元信息表固定 5 行（`业务名 / 服务器 IP / SSH 退出码 / 耗时 / 巡检状态`），不再追加展示型 OS / CPU 型号行。
- **数据来源**：仅消费 `item.business_name` / `item.exit_code` / `item.duration_ms` / `item.inspection_status`（中文映射）与调用方传入的 `host`（由 `resolve_server_ip_map` 反查），不读 `parsed_values`。
- **`inspection_fields` 当前契约**：`disk_used_pct` / `mem_used_pct` / `cpu_idle_pct` / `cpu_iowait_pct` / `swap_used_pct` / `inode_used_pct` / `load_1m` / `io_util_pct` / `io_await_ms`（Linux 9 条）；Windows 8 条少 `cpu_idle_pct` 与 `load_1m` 多 `cpu_used_pct`。**不**含 `os` / `cpu_model` / `uptime_hours`——展示型元数据已从当前脚本契约移除。
- **历史 JSONB 兼容**：旧采集记录的 `parsed_values` 可能仍含 `os` / `cpu_model` 键；当前 `_server_meta_rows` 不消费这些键，吞值不报错，docx 报告不受影响，数据库无需迁移。
- **回归保护**：`app/tests/scripts/ops/test_ops_report.py` 用例 `test_build_report_meta_table_omits_os_and_cpu_rows`（3 场景断言元信息表固定 5 行且不含 OS / CPU 行）+ `test_build_report_meta_table_does_not_render_os_cpu_with_real_values`（断言带字符串值时不渲染，锁防回归）。
...
12. **第三方命令执行器分支（2026-08-03 新增；2026-08-05 改造为 async def）**：`SSHTools.execute_command` 支持通过 `runtime.context["use_third_party_executor"]=True` 切换到走第三方 HTTPS 端点（跳过本地 paramiko）。完整加密 / 路由 / 错误处理契约见 `app/shared/utils/crypto/rsa_aes.py` 与 `app/shared/utils/executor/third_party_executor.py`，与第三方对接规范见 `.trae/documents/第三方接口对接规范.md`。
    - **2026-08-05：** `SSHTools.execute_command` / `execute_batch_commands` / `get_system_logs` 三个 `@tool` 统一改为 `async def`。LangGraph `ToolNode`（在 in-flight asyncio loop 内）直接 `await dispatch(...)` 调第三方 HTTPS，本地 paramiko 用 `asyncio.to_thread` 切线程池。LLM tool schema 不变（只读参数名/类型注解）。并发能力显著提升——多 SSH / 多次 tool_calls 通过 `asyncio.gather` 并发跑，不再因同步阻塞卡整个 event loop。**根因修复：** 旧实现走 `asyncio.run()` / `run_coroutine_threadsafe(...)` 包装异步 `dispatch`，在 LangGraph `ToolNode` 的 in-flight loop 中会触发 `RuntimeError: asyncio.run() cannot be called from a running event loop` 或死锁；改 async 后根本消除包装层。
    - `_emit_log` / `_emit_batch_failure_with_members` 同步改为 `async def`，内部 `await service.emit(evt)`（原同步调用未 await 导致 coroutine 不被驱动，顺带修复潜在日志丢失 bug）。
    - 仅 `execute_command` 涉及第三方分支；`execute_batch_commands` / `get_system_logs` 暂未改造为走第三方（按用户当前需求保持不变）。
    - 加密：请求体用 RSA-OAEP（SHA-256, MGF1-SHA256，v1）+ AES-256-GCM 混合加密（基于 `cryptography` 库）。
    - `AgentContext` 显式声明 `use_third_party_executor: bool = False` / `third_party_endpoint_name: Optional[str] = None` 两个字段（业务方通过 `context_overrides` 注入）。
    - SSH 连接信息（`ip` / `port` / `username` / `password`）从 `DevOpsServerService.get_connection_config` 解密后随业务字段一并加密发出（按第三方契约 `execute接口说明.md`：ip + username 均非空 → 第三方走 SSH 远程执行；否则走第三方本机执行）。
    - 配置：`THIRD_PARTY_EXECUTOR_ENDPOINTS`（JSON 数组，每项含 `name` / `url` / `public_key_pem` / `timeout_seconds` / `enabled`，启动期校验 URL `https://` 与 PEM 可解析）+ `THIRD_PARTY_EXECUTOR_DEFAULT_ENDPOINT`（缺省 `"primary"`）。env 名契约：`settings.ThirdPartyExecutorSettings.endpoints_json` 字段通过 `validation_alias=AliasChoices("endpoints_json", "THIRD_PARTY_EXECUTOR_ENDPOINTS")` 显式映射到 `THIRD_PARTY_EXECUTOR_ENDPOINTS`（字段名默认映射为 `THIRD_PARTY_EXECUTOR_ENDPOINTS_JSON`，与设计契约不一致，故显式声明别名；字段名构造方式同时保留）。**`.env` 加载契约（2026-08-05）**：settings.py 全部 `env_file` 使用基于文件位置推导的绝对路径 `_ENV_FILE_PATH`（`Path(__file__).resolve().parents[3] / ".env"`），不依赖进程 CWD——原 `env_file=".env"` 相对路径在 CWD 非项目根（IDE 调试 / 定时任务 / 运维脚本入口）时整个 `.env` 加载失败，表现为第三方端点报"未配置"。**环境变量污染兜底（2026-08-05）**：pydantic-settings 环境变量优先级高于 `.env` 文件，运行环境（IDE 调试配置 / shell profile）存在污染值 `THIRD_PARTY_EXECUTOR_ENDPOINTS`（空串 / 空数组 `[]` / 非法 JSON / 无 primary 的配置）时 `endpoints_json` 被覆盖；`ThirdPartyEndpointRegistry.load_from_settings` 全局懒加载路径（未注入 settings）在**解析后 0 端点**时通过 `_read_env_file_endpoints_fallback()` 从项目根 `.env` 文件读取原始值**重载**（首版仅空串兜底，实测 loaded_endpoints=[] 场景为 `[]` 污染，2026-08-05 增强为 0 端点即重载）。
    ...
    - **回归保护（2026-08-05 新增）：** `test_ssh_tools_third_party.py` 新增 4 个回归用例
13. **脚本系统接入第三方 SSH（2026-08-16）**：`app.scripts.ops.ops_inspection_sweep` 在保留 paramiko 本地路径的同时，新增"按 schedule / 脚本级开关走第三方 HTTPS 执行器"分支，与 `SSHTools.execute_command` 工具路径**同源不同链路**：
    - **入口契约**：`ops_inspection_sweep.params_schema` 新增 `use_third_party_executor: boolean(default=False)` 与 `third_party_endpoint_name: string(default="")` 两个键；`run()` 把这两个 `script_args` 键透传给 `run_server_ops(context, use_third_party=..., third_party_endpoint_name=...)`。**端点缺失时不静默回落本地 paramiko**（用户拍板"不降级"策略：失败按 crit）。
    - **共享 server_ops 关键字**：`run_server_ops` / `_run_one` 新增 `use_third_party` / `third_party_endpoint_name` 两个 keyword-only 参数；当 `use_third_party=True` 时改 `await asyncio.to_thread(execute_third_party_script, ...)`，本地 paramiko 路径**不**被调用（防止同时持有两路凭据）。`_run_one` 把 `business_name` 注入 config dict（仅供第三方分支 audit / dispatch 字段，paramiko 不读）。
    - **复用层**：`app.shared.utils.executor.third_party_ssh.execute_third_party_script` 是新建的**同步薄壳**（~120 行），复用 `app.shared.utils.executor.third_party_executor.dispatch`（async）+ `normalize_response`（同步）二者的全部能力（端点选取 / RSA-OAEP + AES-256-GCM 加解密 / HTTPS / 异常归一），仅做两件事：① 在 worker 线程内用 `asyncio.run` 驱动 async dispatch（与 `SSHTools.execute_command` 在 LangGraph in-flight loop 不同场景，`asyncio.run` 在 worker 线程内安全）；② 把响应归一为与 `ssh.executor.execute_script` **同形**的 `SSHExecResult`，让 `run_server_ops._run_one` 在两条路径上复用同一套 dataclass 与既有「退出码非 0 → crit」判定逻辑。`ThirdPartyExecutorError` 与所有其他异常**全部封进 `SSHExecResult(success=False, exit_code=1, stderr="third_party:<reason>")`**，不向上抛——这是用户拍板"不降级"策略的稳定基础。
    - **模块属性读取**：`third_party_ssh.py` 不再 `from ... import dispatch, normalize_response`（`from X import name` 会把名称绑定到首次解析的函数对象，后续 `monkeypatch.setattr(tp_module, 'dispatch', AsyncMock(...))` 对它不生效），改为 `import tp_module` + 在调用点用 `tp_module.dispatch(...)` / `tp_module.normalize_response(...)`，使测试 monkeypatch 真实生效。`ThirdPartyExecutorError` 是类引用，可直接 import。
    - **端点名解析顺序**：入参 `endpoint_name` → 为空/None 时 `settings.third_party_executor.default_endpoint`（即 `.env` 的 `THIRD_PARTY_EXECUTOR_DEFAULT_ENDPOINT`，默认 `primary`）；`timeout` 钳制到 `[1, 120]` 区间。
    - **前端契约**：`web/Agent/src/components/TaskSchedulerManager.vue` 新增 `isThirdPartySwitchParamDefinition(key, def)` 识别函数（白名单 `{use_third_party_executor, third_party_endpoint_name}` + type 校验）；`supportedScriptParamDefinitions` / `hydrateScriptArgs` (`buildScriptArgs`) / `addScriptParam` 三处同步加 OR 分支；新模板分支 `v-else-if="paramKey === 'use_third_party_executor'"` 渲染单个 checkbox + `v-else-if="paramKey === 'third_party_endpoint_name'"` 渲染带 placeholder 的 text input；`setScriptParamValue(key, value)` helper 仅处理 boolean/string 两个新键，老 server_list / api_list 控件不受影响。
    - **回归保护**：
        - `app/tests/shared/utils/executor/test_third_party_ssh.py` —— **9 个新用例**：导入/签名形状、`dispatch` 返回归一化结果 / 显式 endpoint 透传 / settings 兜底 / `ThirdPartyExecutorError` 封进 `SSHExecResult`（不抛）/ 未预期异常兜底 / timeout 钳制 `[1, 120]` / 响应 `error` 字段写入 `stderr`。
        - `app/tests/scripts/test_server_ops.py` —— **4 个新用例**：`use_third_party=False` 时 paramiko 路径被调/第三方不被调 / `=True` 时第三方位被调/paramiko 不被调（用 monkeypatch 反向断言）/ `=True` + 第三方失败时**不**静默回落本地 paramiko（核心反向契约）/ `_run_one` 把 `business_name` 注入 config 字典供审计用。
        - `app/tests/scripts/ops/test_ops_inspection_sweep.py` —— **3 个新用例**：`params_schema` 含 `use_third_party_executor` (boolean, default False) + `third_party_endpoint_name` (string, default "") 两键（title / description 非空）/ `run()` 把 `script_args.use_third_party_executor=True`、`third_party_endpoint_name='primary'` 透传给 `run_server_ops`（用 stub 替换 `run_server_ops` 捕获 kwargs）/ `script_args` 缺两键时回落到 `False` / `None`。
        - 既有 12 个 stub 函数（`async def stub_server_ops(context):` / `async def stub(context):` / `async def stub_server(context):` / `async def fake_run_server_ops(context):`）统一加 `(context, **kwargs):` 兼容新增 kwargs（不破坏既有测试）。
    - **out of scope**：未触动 `ssh/executor.py` 本地路径、未触动 `SSHTools.execute_command` 工具路径、未改动 DB schema（`script_args` 已是 JSONB）、未触动 settings.py（端点名解析用既有 `THIRD_PARTY_EXECUTOR_*` 契约）、未引入新 SQL 迁移。
    - **关键反例（消费侧运维行动清单）**：
        1. `THIRD_PARTY_EXECUTOR_ENDPOINTS` 解为空注册表 + `use_third_party_executor=true` → 每台 server 都 crit，`error_message` / `inspection_error` 含"未配置"，**不**触发本地 paramiko（与既有"第三方失败按 crit 不降级"契约一致）。
        2. `third_party_endpoint_name` 拼错 → 同上，crit，不 fallback。
        3. 第三方 cert / TLS 错误 → `ThirdPartyExecutorError` 包装成 `SSHExecResult(success=False, exit_code=1, stderr="third_party:<reason>")`，运维从单行日志 stderr 字段定位。
        4. SSH 子系统在 LangGraph in-flight loop 内直接 `await execute_command(...)` 的回归保护已由 [`test_ssh_tools.py::test_execute_command_works_inside_running_event_loop`](/app/tests/shared/tools/skills/devops/test_ssh_tools.py) + 第三方分支对应 `test_ssh_tools_third_party.py::test_execute_command_third_party_inside_running_loop_no_runtime_error` 覆盖；脚本系统的 `asyncio.run(dispatch)` 在 worker 线程（`asyncio.to_thread` 切线程池）执行，**不**与上述 in-flight loop 冲突。

 `test_execute_command_third_party_logs_loaded_endpoints_on_config_missing` / `..._when_registry_invalid` / `..._on_success` / `..._omits_public_key_pem`；`test_third_party_executor.py` 新增 2 个 `test_endpoint_registry_diagnostic_summary_omits_public_key_pem` / `..._empty_when_no_endpoints`。devops 目录 123 → 127 全绿。
...
- `app/tests/shared/tools/skills/devops/test_ssh_tools_third_party.py` —— 11 个用例（2026-08-03 新增 10 + **2026-08-05 新增 1 个**）：ctx 缺省 / `use_third_party_executor=False` 走本地 paramiko、`=True` 走第三方并跳过本地、`third_party_endpoint_name` 缺省回退 `default_endpoint`、黑名单拦截在第三方分支前生效、第三方 HTTP / timeout / config_missing / unexpected 各类错误归类、`_resolve_server_config` 业务名透传给 dispatch、**`test_execute_command_third_party_inside_running_loop_no_runtime_error` 保护第三方分支 in-flight loop 内直接 await dispatch**。
- `app/tests/shared/utils/executor/test_third_party_executor.py` —— 24 个用例（2026-08-03 新增）：端点注册表（合法 / 非 https 拒绝 / 非法 PEM 拒绝 / disabled / 未配置名 → `ERR_CONFIG_MISSING`）、`dispatch` 加密 body roundtrip / 注入 client / 超时 / HTTP 错误 / 4xx / 非法 JSON / 非 dict 响应 / 未配置端点、响应归一化（成功 / stderr / 缺字段 / 非法 exit_code）、**SSH 连接信息注入**（4 个用例：完整注入 / 缺省不注入 / ip 或 username 缺失不注入 / port 缺失默认 22）、**错误响应解析**（3 个用例：4xx 提取 error / 500 提取 error / 4xx 非 JSON body 不抛双错）。
- `app/tests/shared/utils/crypto/test_rsa_aes.py` —— 10 个用例（2026-08-03 新增）：roundtrip / 不同次加密不同密文 / 错密钥解 / 篡改 ciphertext / 篡改 tag / 缺字段 / 非法 PEM / 非法 base64 / 密钥生成格式 / Unicode payload。

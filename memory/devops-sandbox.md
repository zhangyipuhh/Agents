...
12. **第三方命令执行器分支（2026-08-03 新增）**：`SSHTools.execute_command` 支持通过 `runtime.context["use_third_party_executor"]=True` 切换到走第三方 HTTPS 端点（跳过本地 paramiko）。完整加密 / 路由 / 错误处理契约见 `app/shared/utils/crypto/rsa_aes.py` 与 `app/shared/utils/executor/third_party_executor.py`，与第三方对接规范见 `.trae/documents/第三方接口对接规范.md`。
    - 仅 `execute_command` 涉及；`execute_batch_commands` / `get_system_logs` 暂未改造（按用户当前需求保持不变）。

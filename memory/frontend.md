...
- **Admin 管理**：
  ...
  - `FeishuSettingsManager.vue`：admin 飞书设置三 Tab（应用设置 / 发送策略 / 发送测试），挂载在 UserSettingsDialog 「消息设置」(messaging) → channel「飞书设置」(messaging.feishu) 下，与 `EmailSettingsManager.vue` 平级。三 Tab 走 `.feishu-settings-manager`（flex:1）+ `section[role="tabpanel"]`（flex:1 + min-height:0）+ `.feishu-form`（flex:1 + overflow-y:auto）三级高度链契约，与邮件组件同款。**2026-09-07 风格独立化**：原模板仅复用邮件组件的 scoped 类名（class="email-form" / .policies-layout / .policy-editor / .primary-btn 等），但 Vue scoped CSS 只对带 data-v-xxx 属性选择器的元素生效——飞书组件元素没有邮件组件的 hash，导致 tab 下划线 / alert 配色 / grid 布局 / policy-item 选中态 / badge / chip / focus 光晕等大量样式实际从未生效，仅靠浏览器默认样式呈现"看起来差不多"的假象。修复：① 根容器 / 表单 / empty 状态重命名为 `feishu-*` 前缀（避免与邮件 scoped 样式名耦合）；② 文件末尾追加独立 `<style scoped>` 块逐字镜像 EmailSettingsManager 的视觉规格（含 tablist / tab.active 蓝色 #2563eb / detail-header / 按钮 / alert / policies-layout / policy-editor / form-grid / field-row / focus 光晕等通用类全部保留原名以便未来钉钉/企微等通知渠道复用）；③ 新增飞书独有 `.badge.default`（蓝色 #dbeafe / #1e40af）/ `.badge.disabled`（灰色 #f3f4f6 / #6b7280）/ `.create-channel-btn`（margin-top: 12px; width: 100%;）三段样式；④ 模板移除全部 `class="email-*"` + 内联 `style="margin-top: 12px; width: 100%;"` + `secondary-btn danger` 修饰符（合并入独立 `.danger-btn`，与邮件一致）；⑤ 测试 tab 表单 `.field-row` 两栏结构改为 `.form-field.full` 单栏结构（与邮件发送测试 tab 完全一致）；⑥ `.feishu-form:not(.form-grid)` 单列布局应对 test tab 取消 form-grid 后网格列数问题。`FeishuSettingsManager.spec.js` 同步：3 处旧断言更新（class 名 + 注释）+ 新增「风格独立化」describe 块 5 个测试（共 28 → 31 用例，全绿），含 `<style scoped>` 块存在 / 模板不再出现 email-* 类名 / 飞书独有徽章样式 / 内联 style 已迁出 / danger-btn 数量 ≥ 2 / test tab 单栏结构。**未动** EmailSettingsManager.vue 与其 spec。
